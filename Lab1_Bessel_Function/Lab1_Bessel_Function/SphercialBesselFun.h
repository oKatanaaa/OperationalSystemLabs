...
void SphercialBesselFun::create_bessel_fun() {
	...
	TwoFunction* _x_sin = TwoFunctionFactory::multiply(_x, _sin_t);

	// order*t - x*sin(t) = inner(x, t)
	TwoFunction* _inner_x_t = TwoFunctionFactory::two_twosum(_x_sin, _order_t, ArgIndex::SECOND);
